...
export default class Service {
    #model = null;
    #faceLandsmarksDetection
    
    constructor({ faceLandmarksDetection}) {
        this.#faceLandsmarksDetection = faceLandmarksDetection
    }
    async loadModel() {
        this.#model = await this.#faceLandsmarksDetection.load(
            this.#faceLandsmarksDetection.SupportedPackages.mediapipeFacemesh,
            { maxFaces: 1 }
        );
    };
    ...
    async handBlinked(video) {
      ...
      if (!predictions.length) return false 
        for (const prediction of predictions) {
          ...
          if(leftEAR <= EAR_THRESHOLD) console.log('left blinked')
          if(rightEAR <= EAR_THRESHOLD) console.log('right blinked')
          return blinked
        };
        return false
